...
def degrade(image, style, seed):
    ...
    if style == "scan":
        ...
        image = image.rotate(
            rng.uniform(-1.5, 1.5), Image.Resampling.BICUBIC, fillcolor=(245, 245, 245)
        )
        ...
        ImageDraw.Draw(edge).rectangle(
            (3, 3, size[0] - 4, size[1] - 4), outline=55, width=9
        )
        ...
        paper.thumbnail(
            (int(size[0] * 0.83), int(size[1] * 0.83)), Image.Resampling.LANCZOS
        )
        sheet.paste(
            paper, ((size[0] - paper.width) // 2, (size[1] - paper.height) // 2)
        )
        ...
        image = image.resize(
            (int(size[0] * 0.72), int(size[1] * 0.72)), Image.Resampling.LANCZOS
        )
    ...
    result.paste(
        image.resize((size[0] - 16, size[1] - 16), Image.Resampling.LANCZOS), (8, 8)
    )
    return result
