...
class RealisticTests(unittest.TestCase):
            ...
            for style in ("clean", "scan", "photo", "mixed"):
                ...
                for index in range(1, 3):
                    for suffix in ("png", "json"):
                        ...
                        self.assertEqual(
                            first, (root / style / "b" / name).read_bytes()
                        )
                        if suffix == "json":
                            self.assertEqual(
                                json.loads(first), make_invoice(7, index - 1)
                            )
                        elif style != "clean":
                            self.assertNotEqual(
                                first, (root / "clean" / "a" / name).read_bytes()
                            )
                if style == "scan":
                    with Image.open(root / style / "a" / "invoice-01.png") as image:
                        self.assertEqual(
                            image.getchannel("R").tobytes(),
                            image.getchannel("G").tobytes(),
                        )
    ...
    def test_printed_formats_vary_without_mutating_truth(self):
        ...
        for seed in range(20):
            ...
            def capture(draw, xy, text, *args, printed=printed, **kwargs):
                printed.append(text)
            ...
            dates.update(
                text.removeprefix("Invoice date: ")
                for text in printed
                if text.startswith("Invoice date: ")
            )
            money.update(
                text for text in printed if text in {"1285.58", "1,285.58", "$1,285.58"}
            )
            self.assertEqual(invoice, original)
        self.assertEqual(
            dates, {"2026-11-14", "14 Nov 2026", "Nov 14, 2026", "14/11/2026"}
        )
