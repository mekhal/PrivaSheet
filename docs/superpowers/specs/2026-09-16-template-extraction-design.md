...
- Multi-user access, authentication beyond the local session token, remote access.
...
| # | Decision | Rationale |
|---|---|---|
| D1 | One template = one layout. A vendor may have several templates. | Vendors use more than one invoice layout. |
| D2 | The user picks the template for each scan batch. | Automatic selection is out of scope. |
| D3 | The user defines the field list; the AI only locates fields. | Stable output keys across templates. |
| D4 | The LLM runs on every document, using the template as hints (no coordinate alignment). | Input formats are unpredictable; flexibility is preferred over speed. |
| D5 | The LLM returns OCR box IDs and text spans only; values are derived from OCR text by code. | The LLM cannot introduce text that is not in the document. |
...
One row per `(template_id, version)` in `templates`. A saved version is never updated (`store` only inserts template rows); editing creates `version + 1`.
...
- Reserved keys with arithmetic meaning (§6) must have type `decimal`: fields `subtotal`, `discount`, `shipping`, `tax`, `total`; columns `qty`, `unit_price`, `amount`. Arithmetic uses the table that has an `amount` column.
- Version 1 supports at most one table per template. The `tables` array and the nested export keep the format open for more.
...
```json
{
  ...
  "issues": [
    { "code": "TOTAL_MISMATCH", "target": "total", "detail": "subtotal 1200.00 + tax 84.00 = 1284.00, total 1248.00" }
  ],
  "review": null,
  "error": null,
  "revision": 3,
...
- In a review, `null` means "not on the document", equivalent to `missing` in extraction; the arithmetic rules in §6 treat it the same way.
...
- "Mark reviewed" is a request based on revision *n* whose `acknowledged` lists every issue of that revision and has `revision: n`. It succeeds only if the stored revision is still *n* (§5.4); the status then becomes `reviewed`. Editing values after marking reviewed sets the status back to `needs_review` until it is marked again.
...
One row per batch in `batches`. Uploaded files are first saved to `documents/`, flushed and closed; then the `documents` rows, the `queued` results and the batch row are inserted in **one transaction**, which is the commit point. Files in `documents/` or `pages/` with no database row are orphans and are removed on startup.
...
Written to `exports/<batch_id>.jsonl`. Export is allowed only when the batch is **complete**: no document is `queued`, `processing` or `needs_review`. The file has one line per `passed` or `reviewed` document, in manifest order; `failed` documents are left out and listed in the UI. Running export again rewrites the whole file. Values come from `review` when present, else from `extracted`.
...
2. User names the template and defines fields (key, type, required, date format) and tables (key, columns).
...
```
4. The UI shows the page with every OCR box as an overlay and the proposed assignment. The user reassigns by clicking boxes and may narrow the span to part of a box's text (e.g. `INV-0042` inside `Invoice No: INV-0042`); the span must satisfy the grounding rule in §5.3. The user marks key labels and confirms.
...
1. User picks a template and uploads files. Each file is validated by `ingest`; rejected files are reported immediately and create no job (§7).
2. Each accepted file becomes a result in `queued`. The single worker processes documents one at a time.
3. Per document: `ingest` + `ocr` (child process, reusing an existing snapshot with the same ID) → `layout` → `extractor` → `validate`. The outcome — `extracted`, `issues` and status `passed` or `needs_review` (including a `PROCESSING_TIMEOUT` after 10 minutes, §7), or status `failed` with `error` — is written in **one** result commit (§5.4).
...
OCR text is untrusted input. The LLM has no tools. The prompt states that hints may be inaccurate and that a field not present must be reported as missing.

The LLM must return:

```json
{
  "fields": { "invoice_no": { "box_ids": ["p1-b0007"], "span": "INV-0042" }, "po_no": { "missing": true } },
...
- Every field key in the template must be present, either as `{box_ids, span}` or `{ "missing": true }`.
- Every table key must be present as an array (possibly empty). Every row must contain every column key of that table, each as `{box_ids, span}` or `{ "missing": true }`.
...
- **Deletion** of a batch is refused while any of its documents is `queued` or `processing`.
...
| Code | Check |
|---|---|
| `TEMPLATE_MISMATCH` | Fewer than `match.min_key_label_ratio` of key labels are found in the snapshot. A label is found when a box text, lower-cased and whitespace-normalized, has `difflib` similarity ≥ 0.8 with the label or contains it. The UI suggests creating a new template. |
...
| `LOW_OCR_SCORE` | A cited box has `score` below 0.80. |
| `LINE_AMOUNT_MISMATCH` | For a row: `|qty × unit_price − amount| > tolerance`. |
| `SUBTOTAL_MISMATCH` | `|Σ amount − subtotal| > tolerance`. |
| `TOTAL_MISMATCH` | `|subtotal − discount + shipping + tax − total| > tolerance`. |
| `RECONCILIATION_UNAVAILABLE` | A rule applies (below) but a needed operand is missing or unparsed. |
...
- All arithmetic uses `decimal.Decimal` with a 28-digit context; results are compared without rounding against `rules.tolerance` (default `0.01`).
- `LINE_AMOUNT_MISMATCH` applies when the table has `qty`, `unit_price` and `amount` columns; `SUBTOTAL_MISMATCH` when the table has `amount` and the template has `subtotal`; `TOTAL_MISMATCH` when the template has `subtotal` and `total`.
- In an applicable rule, `qty`, `unit_price`, `amount`, `subtotal` and `total` are required operands: if one is missing or unparsed, the rule raises `RECONCILIATION_UNAVAILABLE` instead of being skipped. `discount`, `shipping` and `tax` count as 0 only when the template does not define them or the extractor reported them `missing`; an unparsed value raises `RECONCILIATION_UNAVAILABLE`.
- The 0.80 OCR score and 0.8 label similarity are starting defaults, tuned by the feasibility spike (§10).
...
| Files per batch | 50 | Before accepting files. |
| Accepted types | PDF, JPEG, PNG, TIFF | File signature, not extension. |
| Pages per document | 10 | PDF page count / TIFF frame count, read without rendering. |
...
Logs record codes, IDs and timings only — never OCR text, values or file contents.
...
- Rejects requests whose `Host` is not `127.0.0.1:<port>` or `localhost:<port>`.
- At startup generates a random session token and opens the browser at `/session?token=…`. That endpoint accepts the token once, sets it in a `SameSite=Strict`, `HttpOnly` cookie, and redirects to `/` so the token leaves the address bar.
- **Every** request except `/session` requires the cookie, including reads of images, results and exports. State-changing requests also require a matching `Origin`.
- Document-derived strings (OCR text, values, file names) are inserted into the page as text only, never as HTML. Stored documents and exports are served with `Content-Disposition: attachment` except page images shown in the UI, which are served as `image/png` re-encoded by the app.
...
- Model name from `PRIVASHEET_MODEL`. The app cannot verify that the LLM server itself runs the model locally (some servers can forward to hosted models). The README states that the user must configure the server with a locally downloaded model.

**OCR models**

- RapidOCR must load models from the installed package or the data directory only. Runtime model downloads are disabled; a missing model is an `OCR_FAILED` error. The feasibility spike verifies this with the network disconnected.

**Storage**

- Data directory from `PRIVASHEET_DATA_DIR`; default `%LOCALAPPDATA%\PrivaSheet` on Windows, `~/.local/share/privasheet` elsewhere.
- Layout: `privasheet.db` (plus SQLite's `-wal` and `-shm` files), `documents/`, `pages/`, `exports/`, `privasheet.lock`.
...
- The database contains extracted invoice data and OCR text and must be treated as sensitive as the documents themselves. It is not encrypted in this design.
- Uploaded files are stored as `documents/<document_id>.<ext>`; original names are kept only as `source_file` metadata.
- Deleting a batch (allowed only when it has no active work, §5.4) deletes, in one transaction and in this order, its results, its batch row, each of its `documents` rows not referenced by a template's `sample_document_id`, and each of its snapshot rows that no remaining result, document or template references. The matching files in `documents/`, `pages/` and `exports/` are deleted after the commit; any left behind by a crash are removed as orphans on startup. There is no automatic retention in this design.

**Configuration** (environment variables): `PRIVASHEET_BASE_URL`, `PRIVASHEET_MODEL`, `PRIVASHEET_DATA_DIR`, `PRIVASHEET_PORT` (default 8765), `PRIVASHEET_DOCUMENT_TIMEOUT_S` (default 600).
...
| Screen | Purpose |
|---|---|
| Templates | List templates and versions; start a new template or a new version. |
| New template | Upload sample → name → define fields and tables → "AI propose" → review page: page image with an SVG overlay of OCR boxes, click to assign or reassign, mark required and key labels → save. |
| Scan batch | Pick template → upload files → per-file progress and status. |
| Review queue | Page image with cited boxes highlighted, issue list, editable values in canonical form (date picker, plain decimals; typed values allowed), add or remove table rows → "Save review" reruns checks (§4.3) → acknowledge remaining issues → "Mark reviewed". |
| Export | Pick a complete batch → download JSONL; failed documents are listed. |
